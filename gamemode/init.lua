	...
	if NextTick <= time then
		...
		for _, pl in pairs(humans) do
			if pl:Alive() then
				...
				if self:GetWave() == 1 and not pl.AlreadyHealed then
					pl.AlreadyHealed = true
				...
				if self:GetWave() >= 1 and time >= pl.BonusDamageCheck + 60 then
					...
					if pl.BuffSurvivor then
						pl:AddPoints(2)
						pl:PrintTranslatedMessage(HUD_PRINTCONSOLE, "bonus_points_added", 2)
					end

					pl.BonusDamageCheck = time
				end

				if pl.BuffRegenerative and time >= pl.NextRegenerate and pl:Health() < 40 then
					pl.NextRegenerate = time + 4

					pl.FutureHealth = pl:Health() + 2
					if pl.FutureHealth > 40 then
						pl:SetHealth(40)
					else
